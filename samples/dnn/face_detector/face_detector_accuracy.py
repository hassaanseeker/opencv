...
parser.add_argument('--prototxt', help='Path to .prototxt of Caffe model')
parser.add_argument('--weights', help='Path to .caffemodel trained in Caffe')
...
### Obtain detections ##########################################################
detections = []
if args.prototxt and args.weights:
    net = cv.dnn.readNetFromCaffe(args.prototxt, args.weights)

    def detect(img, imageId):
        imgWidth = img.shape[1]
        imgHeight = img.shape[0]
        net.setInput(cv.dnn.blobFromImage(img, 1.0, (imgWidth, imgHeight), (104., 177., 123.), False, False))
        out = net.forward()
